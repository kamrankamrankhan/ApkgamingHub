...
import { Heart, Users, Sparkles, Eye } from 'lucide-react';
import { Card, CardContent } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { Game } from '@/lib/games-data';
import { useGameStore } from '@/lib/store';
import { getGameDetailHref } from '@/lib/game-routes';
...
export function GameCard({ game, index = 0 }: GameCardProps) {
  ...
  const href = getGameDetailHref(game);
  ...
  return (
    <motion.div
      ...
      className="group"
    >
      <Card className="relative overflow-hidden bg-gradient-to-br from-purple-900/80 to-purple-950/80 border-purple-700/50 hover:border-yellow-500/50 transition-all duration-300 p-0 gap-0 py-0">
        <Link href={href} className="block cursor-pointer text-left">
          <div className="relative aspect-[4/3] overflow-hidden">
            ...
            >
              <span className="w-14 h-14 rounded-full bg-gradient-to-r from-yellow-500 to-orange-500 flex items-center justify-center shadow-lg shadow-yellow-500/30">
          ...
          <CardContent className="p-3">
            ...
            <div className="flex items-center justify-between mt-1">
              ...
              {game.rtp && <p className="text-xs text-green-400">RTP: {game.rtp}%</p>}
            </div>
        ...
        <motion.button
          ...
          className={`absolute top-2 right-2 z-20 w-8 h-8 rounded-full flex items-center justify-center transition-all ${
            isFavorite ? 'bg-red-500 text-white' : 'bg-black/50 text-white hover:bg-red-500/50'
          }`}
        ...
        <div className="px-3 pb-3">
          <Button
            asChild
            size="sm"
            className="w-full bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-500 hover:to-purple-600 text-white text-xs"
          >
            <Link href={href}>
